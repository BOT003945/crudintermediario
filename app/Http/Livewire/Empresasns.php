...
class Empresasns extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.empresasns.view', [
            'empresasns' => Empresasn::latest()
						...
						->orWhere('pacientesN_Mes', 'LIKE', $keyWord)
						->orWhere('pacientesN_Acum', 'LIKE', $keyWord)
    ...
    {		
		...
		$this->colonia = null;
		$this->descuento = null;
		$this->pacientesN_Mes = null;
		$this->pacientesN_Acum = null;
    ...
    public function store()
    {
        ...
        Empresasn::create([ 
			...
			'pacientesN_Mes' => $this-> pacientesN_Mes,
			'pacientesN_Acum' => $this-> pacientesN_Acum,
		...
		session()->flash('message', 'Empresasn Successfully created.');
    }

    public function edit($id)
    {
        $record = Empresasn::findOrFail($id);

        $this->selected_id = $id; 
		$this->idEmpresa = $record-> idEmpresa;
		...
		$this->pacientesN_Mes = $record-> pacientesN_Mes;
		$this->pacientesN_Acum = $record-> pacientesN_Acum;
    ...
    public function update()
    {
        ...
        if ($this->selected_id) {
			$record = Empresasn::find($this->selected_id);
            $record->update([ 
			'idEmpresa' => $this-> idEmpresa,
			'sucursal' => $this-> sucursal,
			'Nombre' => $this-> Nombre,
            ...
            $this->resetInput();
            $this->updateMode = false;
			session()->flash('message', 'Empresasn Successfully updated.');
        }
    }
